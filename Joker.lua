...
--[[
  ** GetJoke()
  PURPOSE: Holds the various joke, riddle, & other reference compilations and disperses.
  NOTE: Curently, ESO addons are only allowed to interact with saved variables. Instead of polling remote URL (my original plan), we'll just dump a bunch of jokes into a var and index on-demand.
  TODO: As we add more jokes (and more categories of jokes), we'll need to add the option for users to disable specific cats so they aren't loading in a bunch of data each login/reloadui [each var is loaded in]

  @param replaceWithSelf    bool; Instead of Chuck, use our own name instead
]]
function Joker.GetJoke()

  -- Compilation of Joker jokes
  local norrisJokes = {
...
-- Quick utility to check if string is empty
function Joker.isempty(s)
  return s == nil or s == ''
end

function Joker.Get(target)
  local joke = Joker.GetJoke()
...
function Joker.OnAddOnLoaded(event, addonName)
    ...
    SLASH_COMMANDS["/norris"] = Joker.Get
